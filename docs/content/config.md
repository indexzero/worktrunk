...
{{ terminal(cmd="git config worktrunk.config.post-start __WT_QUOT__npm run dev__WT_QUOT__|||git config worktrunk.config.forge.platform github|||git config worktrunk.config.list.url 'http://localhost:__WT_OPEN__ branch | hash_port __WT_CLOSE__'") }}
...
{{ terminal(cmd="git config --add worktrunk.config.step.copy-ignored.exclude .cache/|||git config --add worktrunk.config.step.copy-ignored.exclude .turbo/") }}
...
{{ terminal(cmd="git config --show-origin --get-regexp '^worktrunk\.config\.'") }}
